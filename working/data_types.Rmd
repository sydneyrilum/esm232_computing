...
ggplot(data.frame(catch1=catch1), aes(catch1, fill=catch1)) +
...
# which fish was least frequently caught
...
# can't do
which.min(catch1) # returns name and factor level not catch number
which.min(summary(catch1)) # need summary
summary(catch1)[5] # ID summary of catch #5
```
...
source(here("R/compute_simpson_index.R"))
...
Lets start with diversity - maybe we want to know a bit more about the dataset
...
# repeat with a list
source(here("R/computediversity.R"))
